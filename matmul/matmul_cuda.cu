...
__global__ void kernel_hidden_backward(
    struct matrix lhs,
    struct matrix rhs,
    ...
    for (u32 i = 0; i < rhs.height; ++i)
        v += rhs.data[gy + i * rhs.stride] * parent.grad[gx + i * parent.stride];

    lhs.grad[gx + gy * lhs.stride] = v;
    lhs.data[gx + gy * lhs.stride] -= v * LEARNING_RATE;
}
...
static void hidden_backward(
    ...
    const u32 num_blocks_x = (lhs.width  + num_threads-1u) / num_threads;
    const u32 num_blocks_y = (lhs.height + num_threads-1u) / num_threads;
    const dim3 grid_dims(num_blocks_x, num_blocks_y);

    kernel_hidden_backward<<<grid_dims, block_dims>>>(lhs, rhs, parent);
}
